...
public class AdminUserControler {
    ...
    @ResponseBody
    public Result getUser(@TokenToUser AdminUser user) {
    ...
    public Result updPassword(@RequestBody String password, @TokenToUser AdminUser user) {
        if (adminUserService.updPassword(user,password) > 0) {
            logger.info("请求修改用户成功 " + user.toString());
            return ResultGenerator.genSuccessResult();
        }else {
            return ResultGenerator.genFailResult("添加失败");
